...
def delete_channel_data(channel_id):
    try:
        ...
        mydb = mysql.connector.connect(
            ...
            user="root",
            password="",
            database="youtube_project"  # Your database name
        )
        cursor = mydb.cursor()

        # Delete data from channels table
        delete_channels_query = "DELETE FROM channels WHERE channel_Id = %s"
        cursor.execute(delete_channels_query, (channel_id,))

        # Delete data from videos table
        delete_videos_query = "DELETE FROM videos WHERE Channel_Id = %s"
        cursor.execute(delete_videos_query, (channel_id,))

        # Delete data from playlists table
        delete_playlists_query = "DELETE FROM playlists WHERE Channel_Id = %s"
        cursor.execute(delete_playlists_query, (channel_id,))

        # Delete data from comments table
        delete_comments_query = "DELETE FROM comments WHERE Video_id IN (SELECT Video_Id FROM videos WHERE Channel_Id = %s)"
        cursor.execute(delete_comments_query, (channel_id,))

        # Commit the changes
        mydb.commit()
        
        return "Data related to the channel has been deleted successfully."
...
result = delete_channel_data(channel_id)
print(result)  # Output the result of the deletion process
...
if selected == "Extract and Transform":
    ...
    if  show_table=="CHANNELS" and st.button("show table"):
        show_channels_table()

    if  show_table=="PLAYLISTS" and st.button("show table"):
        show_playlists_table()


    if show_table=="VIDEOS" and st.button("show table"):
        show_videos_table()


    if show_table=="COMMENTS" and st.button("show table"):
        show_comments_table()
    ...
    if st.button("Delete From Sql"):

            delete = delete_channel_data(channel_id)
            st.success(delete)
